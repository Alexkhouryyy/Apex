...
**Generated:** 2026-09-02 · **Basis:** 1678 tests, 14 smoke checks, 3 static audits
...
| 25, 26 | Hand tracking + gesture safety | PARTIAL | **One source, one recognizer** — `agent/handtrack.py` is Apex's own MediaPipe tracker reading the webcam directly (Apache 2.0, no browser). A second source polling a `barehands` server over localhost was built and removed: it never reliably tracked hands on real hardware, and once the native tracker existed it was redundant weight rather than a fallback worth keeping. Shared core in `agent/gestures.py`. The board's own grab logic (`agent/board.py`) now implements the gesture-safety contract this project's design doc called for: a pinch must hold for `ARM_DWELL_SECONDS` before it commits to a grab (a single false-positive tracking frame no longer grabs anything), and an open-palm gesture (`agent/handtrack.is_open_palm`, a generic four-finger-extension geometry check) always cancels a hold and reverts the object to before it began — not wherever it currently sits. Tests: `tests/test_handtrack.py`, `tests/test_board.py` (`TestArmDwell`, `TestOpenPalmCancel`), smoke `hand_tracking_fails_loudly_with_no_camera`. **Calibrated 2026-09-02**, and it found exactly what it was built to find: `scripts/calibrate_pinch.py` run against a real hand measured pinched ratios of 0.25-0.62 and an open hand at 0.83-1.02, so the shipped default of 0.45 sat *below* the top of the real pinch range — every looser pinch was silently classified as not-a-pinch. Default is now 0.70 (`config.HANDTRACK_PINCH_RATIO`). The same run detected a hand in only 46 of 205 frames, and an open hand less often than a pinched one, so `HANDTRACK_MIN_CONFIDENCE` went 0.7 -> 0.5. **Still not WORKS:** those are two settings changed on the strength of one measurement; nobody has yet pinched a card on the board and had it grab. The dwell/cancel logic above remains unproven against a real camera for the same reason. This row moves to WORKS when a pinch moves a card |
