...
**Basis:** 1679 tests, 14 smoke checks, 3 static audits, CI on every push.
...
| 5 | MCP foundation | Discover and use a tool **end-to-end safely** | **HALF MET — the unsafe half.** Discovery and use work (`agent/mcp_client.py`, dispatch at `agent/core.py:2024`). The word "safely" has no implementation: grepping `mcp_client.py` for permission, allowlist, audit, approve or deny returns nothing, and dispatch passes straight through with a single IoT special-case. See below |
...
1. **Phase 5's safety half** — permission tiers, an audit log, and a
   read/write distinction for MCP. Largest live risk, and a prerequisite the
   blueprint itself sequences ahead of 6 and 7.
