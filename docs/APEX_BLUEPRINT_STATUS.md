...
| 6 | Cloud Core | Laptop offline, phone still reaches Apex memory | **BUILT, NOT DEPLOYED.** `agent/relay.py` seals every artefact before it leaves the laptop (`seal()` raises rather than falling back to plaintext) and snapshots through SQLite's online backup API so a WAL-mode database is copied consistently. `relay/server.py` is a stdlib-only mailbox that imports nothing from Apex, binds `127.0.0.1`, and serves nothing at all when no token is configured. `agent/working_context.py` sends a redacted, allowlisted page rather than the archive. `relay/answer.py` lets the cloud answer without the mailbox ever holding a model key. **What is not done is step 9:** no always-on box is running this, so the success check — lid shut, phone still answers — has never been observed. Proven on localhost, not in the world. **2026-09-24:** the "phone" half of that check had no interface — asking meant SSH-ing into the box to run `answer.py`. The relay now serves a phone page (`/phone`: token-gated, hash-pinned CSP, answers rendered as text) with a question queue that `answer.py --watch` claims atomically and answers from the pushed context; driven end to end in a real mobile-sized Chromium against a real relay process |
...
| 8 | Spatial MVP | Pinch, move, rotate, scale work **reliably** | **PINCH PROVEN ON REAL HARDWARE 2026-09-22; ROTATE, SCALE AND "RELIABLY" STILL OPEN.** A real hand in front of a real camera pinched a card on `/board` and it grabbed — the first time that has been observed rather than asserted. What made it checkable was not a code change to the grab path: it was the live readout (`/board?diag=1`, or D), which puts the pinch ratio next to its threshold and names which of five causes is stopping a grab. Being exact about what one grab proves: the single-hand pinch works with the shipped `HANDTRACK_PINCH_RATIO` on this camera and this hand. Rotate and scale are two-handed and have not been tried. "Reliably" is the adjective in the success check, and one success is not a rate. **Measured next: 70% over repeated two-handed attempts, every miss "picked up, then dropped".** Cause found in code, not in the hand: the pinch was one threshold decided afresh every frame, so a pinch hovering near 0.70 flickered and one "open" frame released the card — the module cites a gesture contract of "dwell time, hysteresis" and only dwell had been built. Fixed with a latch (starts below 0.70, ends above 0.78, placed from the calibration so every open-hand reading still lets go) and replayed through the real landmarks-to-board path in tests. **A code review then found three more causes of the same symptom, each reproduced against the real code:** a card was held by its hand's *position in the list*, so a second hand coming into view took it, dropped it or threw it; one frame MediaPipe missed (motion blur mid-drag) dropped the card, or on a brisk drag threw it away; and a one-frame Left/Right label flip reset the new latch. Fixed: hands are followed by where they are (`HandIdentities`), and a hand missing for up to 0.25 s keeps its hold and its pinch. Which of these four causes the 70% actually was is not known. **Re-test 2026-09-23: reported passed by the user against the 18/20-per-hand bar** (G1 in `docs/APEX_V2_PLAN.md`). The per-hand counts were not recorded, so this is a reported pass with an unknown margin, not a measured rate — the first time a number is wanted again, record it |
| 9 | Spatial + Apex | Create, manipulate, close, reopen, continue | **MOSTLY.** Voice-to-scene via `board_create`/`board_recolor` into Blender; versioned assets with provenance (`agent/assets.py`); the board survives a restart, proven by test. The *manipulate* leg now has one observed grab (Phase 8) and inherits its open rotate/scale and reliability questions |
