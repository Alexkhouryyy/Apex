...
(async()=>{try{
  ...
  // Switch mode with an open hand, without returning to the dropdown.
  await page.getByRole('button',{name:'Enable hands',exact:true}).click();
  await page.locator('#hand-mode-bar').waitFor({state:'visible'});
  const rawHand=async(x,y,pinched=false)=>page.request.post(origin+'/api/test/hands',{headers,data:{hands:[{id:7,x,y,pinched,ratio:pinched?.2:.8,threshold:.35}]}});
  const modePoint=await page.locator('[data-hand-mode="orbit"]').evaluate(el=>{const b=el.getBoundingClientRect(),r=document.querySelector('#model').getBoundingClientRect();return {x:(b.x+b.width/2-r.x)/r.width,y:(b.y+b.height/2-r.y)/r.height};});
  await rawHand(modePoint.x,modePoint.y);
  await page.waitForFunction(()=>document.querySelector('#interaction').value==='orbit',{},{timeout:5000}).catch(async e=>{console.error('Mode switch state',await page.evaluate(()=>({status:document.querySelector('#hand-status').textContent,cursor:document.querySelector('#hand-cursor').style.cssText,buttons:[...document.querySelectorAll('[data-hand-mode]')].map(b=>[b.textContent,b.style.cssText])})));throw e;});
  assert.equal(await page.locator('[data-hand-mode="orbit"]').getAttribute('aria-pressed'),'true');
  const viewRevision=(await (await page.request.get(stateUrl,{headers})).json()).revision;
  await rawHand(.5,.5);await page.waitForFunction(()=>document.querySelector('#hand-status').textContent.includes('Ready'));
  await rawHand(.5,.5,true);await page.waitForFunction(()=>document.querySelector('#hand-status').textContent.includes('Holding'));
  await rawHand(.6,.55,true);await page.waitForTimeout(150);await rawHand(.6,.55);await page.waitForTimeout(250);
  assert.equal((await (await page.request.get(stateUrl,{headers})).json()).revision,viewRevision,'camera gesture never edits the component');
  await page.locator('#hand-reach').selectOption('2');
  await rawHand(.6,.65);
  await page.getByRole('button',{name:'Centre here',exact:true}).click();
  await page.waitForFunction(()=>document.querySelector('#comfort-status').textContent.startsWith('Centred'),{},{timeout:7000});
  await page.waitForFunction(()=>Math.abs(parseFloat(document.querySelector('#hand-cursor').style.left)-50)<.1&&Math.abs(parseFloat(document.querySelector('#hand-cursor').style.top)-50)<.1);
  await rawHand(.65,.65);await page.waitForFunction(()=>Math.abs(parseFloat(document.querySelector('#hand-cursor').style.left)-60)<.1);
  await page.getByRole('button',{name:'Reset reach',exact:true}).click();
  await page.waitForFunction(()=>Math.abs(parseFloat(document.querySelector('#hand-cursor').style.left)-65)<.1);
  await page.getByRole('button',{name:'Pause hands',exact:true}).click();
  assert.equal(await page.locator('#hand-mode-bar').isVisible(),false);
  await page.getByRole('button',{name:'Undo study action',exact:true}).click();
  await page.getByRole('button',{name:'Exit isolation',exact:true}).click();
  await page.getByRole('button',{name:'Take apart',exact:true}).click();
  await page.waitForTimeout(800);
