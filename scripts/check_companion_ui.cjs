...
(async()=>{
 ...
 let played=0, audioInstance;
 w.URL.createObjectURL=()=> 'blob:voice';w.URL.revokeObjectURL=()=>{};
 w.Audio=class {constructor(){audioInstance=this;} async play(){played++;} pause(){}};
 $('spoken').checked=true;$('voice').value='voicebox';$('message').value='Local speech';$('send').click();await tick();finish();await tick();
 ttsResolve(new Response(new Blob(['wave'])));await tick();assert.equal(played,1);assert.ok(d.getElementById('companion').classList.contains('speaking'));
